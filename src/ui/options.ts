...
function load(): Saved {
  try {
    ...
    if (raw) return { music: 0.5, sfx: 0.6, ...JSON.parse(raw) };
  } catch {
    // ignore
  }
  return { music: 0.5, sfx: 0.6 };
}
...
export class OptionsMenu {
  ...
  constructor(root: HTMLElement, audio: AudioPlayer, onResume: () => void) {
    const saved = load();
    ...
    const persist = () => {
      try {
        localStorage.setItem(
          STORAGE_KEY,
          JSON.stringify({ music: Number(music.value) / 100, sfx: Number(sfx.value) / 100 }),
        );
      } catch {
        // ignore
      }
